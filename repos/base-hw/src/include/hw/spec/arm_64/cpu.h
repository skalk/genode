...
#define SYSTEM_REGISTER(name, reg, ...) \
...
{
	SYSTEM_REGISTER(Id_pfr0, id_aa64pfr0_el1,
		struct El2 : Bitfield<8, 4> {};
		struct El3 : Bitfield<8, 4> {};
	);

	SYSTEM_REGISTER(Cntfrq_el0, cntfrq_el0);
	...
	SYSTEM_REGISTER(Esr_el1, esr_el1);
	SYSTEM_REGISTER(Far_el1, far_el1);

	SYSTEM_REGISTER(Hcr, hcr_el2,
		struct Rw : Bitfield<31, 1> {};
	);

	SYSTEM_REGISTER(Mair, mair_el1,
		enum Attributes {
			DEVICE_MEMORY          = 0x04,
	...
	SYSTEM_REGISTER(Spsr_el2, spsr_el2);
	SYSTEM_REGISTER(Spsr_el3, spsr_el3);

	SYSTEM_REGISTER(Tcr_el1, tcr_el1,
	...
	SYSTEM_REGISTER(Ttbr0_el1, ttbr0_el1);
	SYSTEM_REGISTER(Ttbr1_el1, ttbr1_el1);

	SYSTEM_REGISTER(Vbar_el1, vbar_el1);

	static inline unsigned current_privilege_level() {
		return Current_el::El::get(Current_el::read()); }

	static void tlb_invalidate_by_asid(Genode::uint16_t asid)
	{
