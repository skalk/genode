...
struct Pci_driver
{
	...
	Pci_driver(Genode::Env &env) : _env(env)
	{
		...
		_pci.with_xml([&] (Xml_node node) {
			node.with_sub_node("device", [&] (Xml_node node)
			{
				node.with_sub_node("pci-config", [&] (Xml_node node)
				{
				...
				node.with_sub_node("io_mem", [&] (Xml_node node)
				{
					_mmio.construct(_dev);
					_pci_info.io_mem_addr = (addr_t)_mmio->local_addr<void>();
				});

				node.with_sub_node("io_port", [&] (Xml_node node)
				{
