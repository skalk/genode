                     ...
                     void                       *data)
{
	...
	genode_usb_ack_request(handle, handle_return_code(ret), &ret);
}
...
static inline void check_for_urbs(struct usb_device * udev)
{
	if (udev)
		genode_usb_request_by_bus_dev(udev->bus->busnum, udev->devnum,
