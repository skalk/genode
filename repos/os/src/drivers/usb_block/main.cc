...
class Usb::Block_driver : Device::Policy
{
	private:
		...
		void _report_device(char const *vendor, char const *product,
		...
		void _no_write(char *, size_t) { }
		void _no_read(char const *, size_t) { }

		void _inquiry(char *dst, size_t) {
			Inquiry inq((addr_t)dst, INQ_TAG, _active_lun, _verbose_scsi); }

		void _inquiry_result(char const * src, size_t)
		{
			using Response = Scsi::Inquiry_response;

			Response r((addr_t)src, _verbose_scsi);
		...
		void _unit_ready(char *dst, size_t) {
			Test_unit_ready r((addr_t)dst, RDY_TAG, _active_lun, _verbose_scsi); }

		void _sense(char *dst, size_t) {
			Request_sense r((addr_t)dst, REQ_TAG, _active_lun, _verbose_scsi); }

		void _sense_result(char const *src, size_t)
		{
			using namespace Scsi;

			Request_sense_response r((addr_t)src, _verbose_scsi);
		...
		void _start_stop(char *dst, size_t) {
			Start_stop r((addr_t)dst, SS_TAG, _active_lun, _verbose_scsi); }

		void _capacity(char *dst, size_t)
		{
			if (_force_cmd_16)
				Read_capacity_16 r((addr_t)dst, CAP_TAG, _active_lun,
				                   _verbose_scsi);
			else
				Read_capacity_10 r((addr_t)dst, CAP_TAG, _active_lun,
				                   _verbose_scsi);
		}

		void _capacity_result(char const *src, size_t)
		{
			if (_force_cmd_16) {
				Scsi::Capacity_response_16 r((addr_t)src, _verbose_scsi);
				_block_count = r.last_block() + 1;
				_block_size  = r.block_size();
			} else {
				Scsi::Capacity_response_10 r((addr_t)src, _verbose_scsi);
		...
		void _block_write(char *dst, size_t size)
		{
			if (_block_cmd.constructed())
				memcpy(dst, _block_cmd->address, size);
		}

		void _block_read(char const *src, size_t size)
		{
			if (_block_cmd.constructed())
				memcpy(_block_cmd->address, src, size);
		}

		void _block_command(char *dst, size_t)
		{
			if (!_block_cmd.constructed())
				return;
			...
			if (op.type == Operation::Type::READ) {
				if (_force_cmd_16)
					Read_16 r((addr_t)dst, _active_tag, _active_lun,
					          n, c, _block_size, _verbose_scsi);
				else
					Read_10 r((addr_t)dst, _active_tag, _active_lun,
					          (uint32_t)n, c, _block_size, _verbose_scsi);
			} else {
				if (_force_cmd_16)
					Write_16 w((addr_t)dst, _active_tag, _active_lun,
					           n, c, _block_size, _verbose_scsi);
				else
					Write_10 w((addr_t)dst, _active_tag, _active_lun,
					           (uint32_t)n, c, _block_size, _verbose_scsi);
			}
		}
		...
		struct Scsi_command
		{
			...
			void (Block_driver::*cmd)   (char*, size_t);
			void (Block_driver::*read)  (char const*, size_t);
			void (Block_driver::*write) (char*, size_t);
	...
	public:
		                ...
		                Request_stream::Payload const &payload)
		{
			...
			payload.with_content(block_request, [&](void *addr, size_t sz) {
				_block_cmd.construct(*this, block_request, addr, sz,
				                     _new_tag()); });
