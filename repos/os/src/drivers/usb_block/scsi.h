...
struct Scsi::Inquiry_response : Genode::Mmio<0x24>
{
	...
	Inquiry_response(Byte_range_ptr const &range, bool verbose)
	: Mmio(range) { if (verbose) dump(); }
...
struct Scsi::Request_sense_response : Genode::Mmio<0x13>
{
	...
	Request_sense_response(Byte_range_ptr const &range, bool verbose)
	: Mmio(range) { if (verbose) dump(); }
...
struct Scsi::Capacity_response_10 : Genode::Mmio<0x8>
{
	...
	Capacity_response_10(Byte_range_ptr const &range, bool verbose)
	: Mmio(range) { if (verbose) dump(); }
...
struct Scsi::Capacity_response_16 : Genode::Mmio<0xc>
{
	...
	Capacity_response_16(Byte_range_ptr const &range, bool verbose)
	: Mmio(range) { if (verbose) dump(); }
