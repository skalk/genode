			...
			_for_each_packet([&] (Constructible<Packet_descriptor> &cp) {
				...
				if (addr)
					addr = (void*)(genode_shared_dataspace_local_address(&_ds) +
					               ((addr_t)addr - (addr_t)_tx.sink()->ds_local_base()));
	...
	private Dynamic_rom_session::Xml_producer
{
	private:
		...
		bool _controls(genode_usb_device const & device);
		bool _matches(genode_usb_device const & device);

		Device_capability _acquire(genode_usb_device const & device);
...
void genode_usb_device::generate(Xml_generator & xml) const
{
                                  ...
                                  void                             *opaque_data)
{
	...
	case P::SET_INTERFACE:
		_interfaces.for_each([&] (Interface_component & ic) {
			if (ic._iface_idx == cpd->value) granted = true; });
		if (granted) break;
		[[fallthrough]];
	...
	if (!granted) {
		uint32_t ret = 0;
		handle_response(handle, OK, &ret);
		if (_warn_once) {
			warning("Ignore restricted control URB to device ", _device_label,
...
Interface_capability Device_component::acquire_interface(uint8_t index, size_t buf_size)
{
...
bool Device_component::request(genode_usb_req_callback_t const callback,
                               void                           *opaque_data)
{
	bool ret = Base::request(callback, opaque_data);
...
bool Session_component::_controls(genode_usb_device const & d)
{
	...
	_device_policy(d, [&] (Xml_node dev_node) {
		...
		if (!ret) ret = dev_node.attribute_value("controls", false);
	});
	return ret;
}
...
Device_capability Session_component::_acquire(genode_usb_device const & device)
{
                                             ...
                                             Affinity const &)
{
	...
	try {
		...
		sc = new (md_alloc())
			Session_component(_env, _sessions, _devices, _config, _sigh_cap,
			                  _alloc_fn, _free_fn, label,
