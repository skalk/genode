...
class Vdi::File: public Vfs::Io_response_handler
{
	private:
		...
		static inline uint32_t sector_to_block(uint64_t const nr)
		{
		...
		void _sync_header(uint32_t const id)
		{
			...
			if (_state_fs.state == SYNC_HEADER) {
				...
				/* update block table */
				_state_fs.block_nr = 0; /* XXX not available nor used in this context */
				_state_fs.written  = 0;
			...
			if (_state_fs.state == SYNC_HEADER_1) {
				_write(reinterpret_cast<char *>(&table[id]), sizeof(uint32_t), offset);
		...
		void _write(char * const base, Genode::size_t const base_size,
		            Genode::uint64_t const fs_offset)
		{
			do {
				...
				Genode::size_t rest = Genode::min(base_size - _state_fs.written,
		...
		void _read(char * dst, Genode::size_t const dst_size)
		{
		...
		Response _read_split(::Block::Operation const operation,
		                     void * dst, Genode::size_t dst_size,
